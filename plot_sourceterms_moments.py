...
def plot_frame(prim, mcmom, mcsrc, logpath, basename, cycle):
    ...
    formatlegend = [Line2D([1], [0], color='c', marker='o', linestyle="None", label=r'Flux radiative acceleration'), Line2D([1], [0], color='orange', marker='o', linestyle="None", label=r'Scattering radiative acceleration'), Line2D([1], [0], color='k', marker='o', linestyle='-', label='Mean value'), Line2D([1], [0], color='k', marker='o', linestyle='--', label='RMS value')]
...
if __name__ == "__main__":
    ...
    for cycle in range(nfiles):
        ...
        prim = athdf(args.basename+'.out1.'+cyclestr, quantities=["rho", 'r0'])
        mcmom = athdf(args.basename+'.out2.'+cyclestr, quantities=["Ermc", "Frmc1", "rho"])
